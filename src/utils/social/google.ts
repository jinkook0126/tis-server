...
export async function getGoogleAccessToken({ clientId, clientSecret, code, redirectUri }: GetGoogleAccessTokenParams) {
...
export async function getGoogleProfile(accessToken: string) {
  ...
  const socialProfile: SocialProfile = {
    email: data.emailAddresses![0].value || 'nomail',
    name: data.names![0].displayName || 'noname',
